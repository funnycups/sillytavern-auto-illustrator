...
- **Stopping generation in Independent API mode still calls the LLM on partial content** - Fixed bug where hitting Stop mid-stream would still trigger a Luker prompt-gen call on the aborted partial text
  - Root cause: `stopGeneration()` only sets `isFinished=true` (not `isStopped`), so Luker's `isStreamFinished` gate stays true and routes the aborted stream through `onFinishStreaming` → `MESSAGE_RECEIVED`. Independent API mode then sent the partial to the LLM
  - Solution: Listen for `GENERATION_STOPPED` and latch a single-shot flag. The next `MESSAGE_RECEIVED` consumes it; in Independent API mode it skips LLM prompt-gen for the aborted message. Shared API mode ignores the flag but still consumes it, keeping the in-flight image pipeline untouched (per contract: "内联生成不感知任何变化")
