...
- **Stopping generation in Independent API mode still calls the LLM on partial content** - Fixed bug where hitting Stop mid-stream would still trigger a Luker prompt-gen call on the aborted partial text, and its follow-on regression where the very next fresh reply was also skipped after any stop
  - Root cause (stop still calls LLM): Luker's `Generate()` routes aborted streams through `onFinishStreaming` → `MESSAGE_RECEIVED` (`script.js:6498-6544`), so the extension could not tell "finished naturally" from "user aborted" without additional information
  - Root cause (skip leaks to next reply): Non-streaming stops never emit MESSAGE_RECEIVED at all — `stopGeneration()` (`script.js:9160-9192`) aborts the fetch, and `Generate()`'s `onError` (`script.js:9128-9153`) only unblocks and rethrows. Any flag latched on `GENERATION_STOPPED` had no consumer and would linger, false-skipping the next fresh MESSAGE_RECEIVED in Independent API mode. This was the "stop one message and no further images get generated" symptom users reported
  - Solution: Read `context.streamingProcessor.abortController.signal.aborted` directly inside `handleMessageReceived`. Each generation owns its own `AbortController`, so the check is single-shot by construction — no cross-event state, nothing to leak. Mirrors Luker's own memory-graph pattern (`extensions/memory-graph/main.js:15116-15128`). Shared API mode does not check the signal, preserving the "内联生成不感知任何变化" contract for in-flight images scheduled from prompt tags detected during streaming
